...
def query_entity_heatmap(source: CommandSource, ai_prefix: str, top: int = 5):
    ...
    # Scarpet 脚本（已对照 fabric-carpet 官方文档修正）：
    # 1. entity_list('*') 取当前维度所有实体
    # 2. query(e, 'pos') 返回 list（三元组），用 p:0/p:2 访问分量
    # 3. for 对 map 遍历只传 key，取 value 需 map[key]
    # 4. 不用 sort（Scarpet 不支持自定义比较器），改遍历输出实体较多的区块
    script = (
        "es = entity_list('*'); "
        "total = length(es); "
        ...
        "  types[t] = (types[t] ?? 0) + 1 "
        ")); "
        # 按区块统计：query(e,'pos') 返回 list，用 p:0/p:2 取 x/z 分量
        "chunks = m(); "
        "for(es, e -> ( "
        "  p = query(e, 'pos'); "
        "  cx = floor(p:0 / 16); "
        ...
        ")); "
        # 输出类型分布：for 对 map 只传 key
        "print('--- 按类型分布 ---'); "
        "for(types, k -> print(k + ': ' + types[k])); "
        # 输出实体较多的区块（>=5个实体的，避免刷屏）
