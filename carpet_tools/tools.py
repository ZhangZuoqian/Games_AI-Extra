...
@register_tool(description="获取当前服务器中所有假人的列表。当用户问'有哪些假人'、'假人列表'时使用此工具。", parameters={
    "type": "object",
    "properties": {},
    "required": []
})
def list_bot(source: CommandSource, ai_prefix: str):
    server = source.get_server()
    result = server.execute_command("/player list")
    match = re.search(r'Players:\s*\[(.*?)\]', result)
    if match:
        players = [p.strip() for p in match.group(1).split(',') if p.strip()]
        if players:
            return f"当前有 {len(players)} 个假人: {', '.join(players)}"
        else:
            return "当前没有假人"
    match2 = re.search(r'There are \d+ players?:\s*(.*?)$', result, re.IGNORECASE)
    if match2:
        players = [p.strip() for p in match2.group(1).split(',') if p.strip()]
        if players:
            return f"当前有 {len(players)} 个假人: {', '.join(players)}"
    
    return f"无法解析假人列表，原始输出:\n{result}"

@register_tool(description="检查指定的假人是否存在。在操作假人之前，应先使用此工具确认假人存在，避免操作不存在的假人导致错误。", parameters={
    "type": "object",
    "properties": {
        "name": {
            "type": "string",
            "description": "要检查的假人名称"
        }
    },
    "required": ["name"]
})
def check_bot(source: CommandSource, ai_prefix: str, name: str):
    server = source.get_server()
    result = server.execute_command("/player list")
    match = re.search(r'Players:\s*\[(.*?)\]', result)
    if match:
