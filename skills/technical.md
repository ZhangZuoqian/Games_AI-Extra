...
> [注意] **使用前请阅读本 skill**！调用 `read_skills("technical.md")` 获取完整说明。
...
> [注意] 修改前先 `carpet_rule_get` 查询，修改后告知用户如何回滚（设回原值）
...
> [注意] 强加载区块占用算力，建议只加载必要区域（如刷怪塔 16×16 即可）
...
set_tickrate(rate=0)          # 暂停（[注意] 谨慎）
```

> [注意] 调试完务必 `set_tickrate(rate=20)` 恢复正常
...
> [注意] **前置条件**：需服务器已用对应 criterion 创建计分项，且 objective 名与统计项 key 一致。例如查在线时长需先执行 `scoreboard objectives add play_time minecraft.play_time`。如未创建，查询时服务器会报错提示。
...
> [注意] **限制**：vanilla `forceload query` 只能列出由 `forceload` 命令添加的区块。玩家视野加载、spawn 常加载区块、传送门 ticket 等其他加载来源**无法列出**——这是 Minecraft 引擎限制，非本工具问题。
...
| `convert_dimension_pos` | [通过] | [通过] | [通过]（已用） |
| `query_death_log` | [通过] | [通过] | [待测] 待实测（v0.3.2 修复误判逻辑后未在真实服验证） |
| `set_tickrate` | [通过] | [通过] | [通过]（已用） |
| `clear_entities` | [通过] | [通过] | [通过]（已用） |
| `scoreboard_*` | [通过] | [通过] | [通过]（已用） |
| `query_player_stats` | [通过] | [通过] | [待测] 待实测（依赖 scoreboard 计分项是否创建） |
| `query_forceload_detail` | [通过] | [通过] | [待测] 待实测（跨维度输出格式） |
| `query_entity_heatmap` | [通过] | [通过] | [待测] 待实测（Scarpet 脚本语法） |
