...
| `count_entities` | 统计实体数量（按类型/区域） | `center_pos?`, `radius?`, `entity_type?` |
...
| `region_snapshot` | 历史结构截图查询/恢复 | `action`, `pos?`, `snapshot_id?` |
...
# 统计某区域掉落物
count_entities(center_pos=[100, 64, -50], radius=64, entity_type="item")

# 统计某区域僵尸
count_entities(center_pos=[100, 64, -50], radius=32, entity_type="minecraft:zombie")
```

**用途**：排查刷怪塔是否卡顿、农场掉落物堆积、实体上限。

---

## 3. Carpet 规则管理（`carpet_rule_get` / `carpet_rule_set`）
...
## 6. 维度坐标换算（`convert_dimension_pos`）
...
死亡记录由插件自动监听 `player_death` 事件写入 `config/games_ai_extra/death_log.json`。

---

## 8. Tick 速率调整（`set_tickrate`）
...
## 10. 历史结构截图（`region_snapshot`）

```
# 列出所有快照
region_snapshot(action="list")

# 查询某坐标的快照
region_snapshot(action="query", pos=[100, -50])

# 恢复某快照（⚠️ 破坏性操作！）
region_snapshot(action="restore", snapshot_id="snap_20260810_1")
```

> 依赖 region_snapshot 类 MCDR 插件。`restore` 会覆盖当前区块，操作前务必确认！

---

## 11. 计分板管理（`scoreboard_*`）
...
2. 如果 MSPT 高 → `count_entities()` — 看实体数量
3. 如果掉落物多 → `clear_entities(entity_type="item")` — 清理
4. 检查 forceload → `forceload(action="query")`
