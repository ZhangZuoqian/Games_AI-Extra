...
> [注意] **使用前请阅读本 skill**！调用 `read_skills("economy.md")` 获取完整说明。
...
- 工具不检测经济插件装没装，直接发 `balance`/`pay` 命令，没装的话服务端会提示未知命令
- 价格表只存内存，重启清空，跟服务器的实际商店价格不一定一样，注意说明
- 转账是真金白银，调之前跟用户确认金额和收款人
